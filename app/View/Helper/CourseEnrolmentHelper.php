...
class CourseEnrolmentHelper extends AppHelper { 
   ...
   public function getEnrolments($course_id) { 
        ...
        $output = $this->Enrolment->find('all', array(
                'fields' => array('Enrolment.id', 'Enrolment.user_id', 'Enrolment.course_id', 'User.id', 'User.first_name', 'User.last_name', 'User.dietary_requirements', 'User.medical_requirements', 'Enrolment.role'),
				'contain' => array('User'),
				'conditions' => array(
					"course_id" => $course_id,
				)
